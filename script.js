$(document).ready(function() {
	...
	let sections = [...document.querySelectorAll("div .card")];		// An array of all the sections as a whole
	...
	(function enableFunctionality() {
		$("#tableRow").click(addTableRow);		// Adds a row to the bottom of the last row of active sections

		// This will be a calculation made for the resizable grid - it will scale accordingly, depending on the window width/height (Used for a responsive grid on draggable elements)
		// let windowWidth = $(window).width();
		// $(window).resize(function() {
			// 	if($(this).width() != windowWidth) {
				// 		windowWidth = $(this).width() / 10;
				// 		windowWidth = $(this).width();
			//  }
		// });

		// On double click of card-headers, a textbox is added for the user to input sections "Title"
		// $("div .card-header").dblclick(sectionHeaderInteraction);
		...
		$("#previewButton").click(function() {
			...
			const documentBodyStartHTML = $(document.documentElement).html().indexOf("<body>") + 6;
			const docuemntBodyEndHTML = $(document.documentElement).html().lastIndexOf("</div>") + 6;
			...
			$(documentBodyHTML).find(".options-container").remove();
	...
	// Used in conjunction with "update" function to edit the header text
	function sectionHeaderInteraction(event) {
		const headerLink = this.firstElementChild.firstElementChild;

		$(this).unbind("dblclick");
		const tempText = $.trim($(headerLink).text());
		$(headerLink).text("");
		$(headerLink).append(`<textarea class="col-lg-4 text-center">${tempText}</textarea>`);
		$(headerLink).children().unbind("focusout", update).focusout(update);		// Retrieve the textarea created above and add a "focusout" event listener
	}

	// Update the HTML content of the parent element of given element(textarea)
	function update(event) {
		$(this).parent().append($(this).val());		// Adds the text from the textarea into the link that contains textarea and has parent of h5
		$(this).parent().parent().parent().unbind("dblclick", sectionHeaderInteraction).dblclick(sectionHeaderInteraction);		// Place double click event listener on whole header
		$(this).remove();		// Removes textarea element from the DOM (Document Object Model)
	}
	...
	// // Used in conjunction with "updateTextboxText" function to edit the textbox text
	// function textboxInteraction() {
	// 	$(this).unbind("dblclick");
	// 	const tempText = $.trim($(this).text());
	// 	$(this).text("");
	// 	$(this).append(`<textarea class="col-lg-12 text-justify" style="height: 10rem;">${tempText}</textarea>`);
	// 	$(this).children().focusout(updateTextboxText);
	// }

	// // Updates the HTML content of the parent element of the given element (textarea)
	// function updateTextboxText() {
	// 	$(this).parent().append($(this).val());
	// 	$(this).parent().dblclick(textboxInteraction);
	// 	$(this).remove();
	// }

	// // Used in conjunction with "updateMessage" function to edit the message text
	// function messageInteraction() {
	// 	const messageTypeHTML = hasClass(this, "important-message-container") ? `<span style="text-decoration: underline; color: red;">IMPORTANT:</span> ` : `<span style="color: #b1b13c; text-decoration: underline;">NOTE:</span> `;
	// 	$(this).unbind("dblclick");
	// 	const tempText = hasClass(this, "important-message-container") ? $.trim($(this).text().substring(11)) : $.trim($(this).text().substring(7));
	// 	$(this).text("");
	// 	$(this).append(`${messageTypeHTML}<textarea class="important-text col-lg-6">${tempText}</textarea> -->`);
	// 	$(this).children().focusout(updateMessage);
	// }

	// // Updates the HTML content of the parent element of the given element (textarea)
	// function updateMessage() {
	// 	$(this).parent().append($(this).val());
	// 	$(this).parent().dblclick(messageInteraction);
	// 	$(this).remove();
	// }

	// // Used in conjunction with "updateTableTextbox" function to edit the table textbox text
	// function tableTextboxInteraction() {
	// 	$(this).unbind("dblclick");
	// 	const tempText = $.trim($(this).text());
	// 	$(this).text("");
	// 	$(this).append(`<textarea class="col-lg-12 textbox-textarea">${tempText}</textarea>`);
	// 	$(this).children().focusout(updateTableTextbox);
	// }

	// // Updates the HTML content of the parent element of the given element (textarea)
	// function updateTableTextbox() {
	// 	$(this).parent().append($(this).val());
	// 	$(this).parent().dblclick(tableTextboxInteraction);
	// 	$(this).remove();
	// }

	// // Used in conjunction with "updateListText" function to edit the list text
	// function listInteraction() {
	// 	$(this).unbind("dblclick");
	// 	const tempText = $.trim($(this).text());
	// 	$(this).text("");
	// 	$(this).prepend(`<textarea class="list-textarea col-lg-12">${tempText}</textarea>`);

	// 	this.firstElementChild.addEventListener("focusout", updateListText);
	// }

	// // Updates the HTML content of the parent element of the given element (textarea)
	// function updateListText() {
	// 	$(this).parent().append($(this).val());
	// 	$(this).parent().dblclick(listInteraction);
	// 	$(this).remove();
	// }

	// // Used in conjunction with "updateElementText" function to edit the element text - IMPORTANT: takes a string parameter meant to contain a replacementElementHTML that is used to append to the double-clicked element (should be a textarea that allows user to edit/change text in element)
	// function elementInteraction(originalElement, replacementElementHTML) {
	// 	$(originalElement).unbind("dblclick");
	// 	const tempText = $.trim($(originalElement).text());
	// 	$(originalElement).empty();

	// 	console.log(  originalElement  );

	// 	// $(originalElement).append($(replacementElementHTML));
	// 	console.log(  originalElement  );

	// 	$(originalElement).first().append(tempText);
	// 	$(originalElement).first().focusout(function() {
	// 		updateElementText(this, replacementElementHTML);
	// 	});

	// 	updateTableOptions();
	// }

	// // Updates the HTML content of the parent element of the given element (textarea) - IMPORTANT: takes a string parameter meant to contain a replacementElementHTML for "elementInteraction" function
	// function updateElementText(originalElement, replacementElementHTML) {
	// 	$(originalElement).parent().parent().append($(originalElement).val());
	// 	$(originalElement).parent().parent().unbind("dblclick").dblclick(function() {
	// 		elementInteraction(this, replacementElementHTML);
	// 	});
	// 	$(originalElement).parent().remove();
	// }
	...
	function addNewSection() {
		...
		$(`<div class="card">
           		<div class="card-header" role="tab">
                    <h5 class="mb-0">
                        <a contenteditable class="collapsed header" data-toggle="collapse" data-parent="#accordion" href="#collapse${newSectionIDNumber}" aria-expanded="false" aria-controls="collapse${newSectionIDNumber}">
                            <!-- <textarea class="col-lg-4 text-center new-section"></textarea> -->
                        </a>
                    </h5>
                    <div class="options-container col-lg-3 pull-right">
	...
	function addTextbox() {
		...
		if(activeSection === undefined)
			...
			const activeSectionChildren = $(activeSection).children();
			    ...
			    $(`<div class="row text-center textbox-container" style="margin-top: 1.5rem !important;">
			    		<div contenteditable class="textarea-container col-lg-11 text-justify">
		...
		if(activeSection === undefined)
	        ...
	        $(`<div class="table" style="margin-top: 1.5rem !important;">
	           	<div class="row row-text">
	                ...
	                <div class="options-container col-lg-2 table-options">
	                    <span class="option-icon row-down"><i class="fa fa-arrow-down" aria-hidden="true"></i></span>
	                    <span class="option-icon row-up"><i class="fa fa-arrow-up" aria-hidden="true"></i></span>
	                    <span class="option-icon text-edit"><i class="fa fa-font" aria-hidden="true"></i></span>
	                    <span class="option-icon row-delete"><i class="fa fa-trash" aria-hidden="true"></i></span>
	                </div>
	            </div>
	            <div class="row row-text">
	                <div contenteditable class="col-lg-3 text-center table-value">
	...
	// Adds a table row to the bottom of last table row in active / open sections
	function addTableRow() {

		const openSection = getActiveSection();		// Gets the active sections

		// If there is no open sections found then display error message - else - look for new table row
		if(openSection === undefined)
			...
			// If there is no table found in the sections display an error message - else - add a new table row (draggable) to the end of last row
			if(table === undefined)
				displayErrorMessage("PLEASE CREATE A TABLE BEFORE ATTEMPTING TO ADD ROWS", "tableRowError-noTableFound");
			else {
				let tableRows = table.getElementsByClassName("row-text");
				$(`<div class="row row-text">
                   		<div contenteditable class="col-lg-3 text-center table-value">
                        	<!-- <textarea class="col-lg-12 text-center table-textbox"></textarea> -->
                        </div>
	...
	function addImageBlock() {
		...
		if(activeSection === undefined)
			...
			const activeSectionChildren = $(activeSection).first().children();
			    ...
			    $(`<div class="row col-lg-12" style="border: none; margin-top: 1.5rem !important;">
			       		<div class="list-image row text-center col-lg-10">
			       			<div class="list-image col-lg-12">
			                	<input class="upload-image" type="file" />
			            	</div>
			        	</div>
			        	<div class="options-container col-lg-2 image-options">
		...
		if(activeSection === undefined)
			...
			switch($(this).attr("id")) {
				case "importantMessage":
					$(`<div class="row message-block-container col-lg-11" style="margin-top: 1.5rem !important;">
		               		<p class="important-message-container col-lg-11 text-center alert alert-danger"><span class="col-lg-2" style="text-decoration: underline; color: red;">IMPORTANT:</span> <span contenteditable class="col-lg-10"><!-- <textarea class="important-text col-lg-6"></textarea> --></span></p>
		                    <div class="options-container col-lg-1">
		                    	<span class="option-icon text-edit"><i class="fa fa-font" aria-hidden="true"></i></span>
		                    	<span class="option-icon col-lg-12 element-delete"><i class="fa fa-trash" aria-hidden="true"></i></span>
		                    </div>
		               </div>`).appendTo( (activeSectionChildren.length > 0 && !(hasClass(activeSectionChildren[activeSectionChildren.length - 1], "table")) && !(hasClass(activeSectionChildren[activeSectionChildren.length - 1], "important-message-container")) && !(hasClass(activeSectionChildren[activeSectionChildren.length - 1], "note-message-container"))) ? activeSection.firstElementChild : activeSectionChildren[activeSectionChildren.length - 1]);
					break;
				case "noteMessage":
					$(`<div class="row message-block-container col-lg-11" style="margin-top: 1.5rem !important;">
		               		<p class="note-message-container col-lg-11 text-center alert alert-warning"><span class="col-lg-2" style="color: #b1b13c; text-decoration: underline;">NOTE:</span> <span contenteditable class="col-lg-10"><!-- <textarea class="note-text col-lg-6"></textarea> --></span></p>
		                    <div class="options-container col-lg-1">
		                    	<span class="option-icon text-edit"><i class="fa fa-font" aria-hidden="true"></i></span>
		                    	<span class="option-icon col-lg-12 element-delete"><i class="fa fa-trash" aria-hidden="true"></i></span>
		                    </div>
			...
			switch($(this).attr("id")) {
				case "numericList":
					$(`<ol class="ordered-list text-justify col-lg-12" style="margin-top: 1.5rem !important;">
							<li class="col-lg-3 text-justify col-lg-7">
								<div class="row text-justify col-lg-12 list-content-container">
									<div contenteditable class="col-lg-8 col-sm-6 text-justify list-value">
										<!-- <textarea class="list-textarea col-lg-12"></textarea> -->
									</div>
									<div class="options-container col-lg-4 col-sm-5 list-options">
					...
					$(`<ol class="ordered-list text-justify col-lg-12" type="A" style="margin-top: 1.5rem !important;">
							<li class="col-lg-3 text-justify col-lg-7">
								<div class="row text-justify col-lg-12 list-content-container">
									...
									<div class="options-container col-lg-4 col-sm-5 list-options">
										<span class="option-icon col-lg-2 col-sm-1 list-item-next"><i class="fa fa-plus" aria-hidden="true"></i></span>
										<span class="option-icon col-lg-2 col-sm-1 list-item-image"><i class="fa fa-image" aria-hidden="true"></i></span>
										<span class="option-icon col-lg-2 col-sm-1 list-item-unordered-list"><i class="fa fa-list-ul" aria-hidden="true"></i></span>
										<span class="option-icon col-lg-2 col-sm-1 list-item text-edit"><i class="fa fa-font" aria-hidden="true"></i></span>
							        	<span class="option-icon col-lg-2 col-sm-1 list-item-delete"><i class="fa fa-trash" aria-hidden="true"></i></span>
							        </div>
							    </div>
							</li>							
					   </ol>`).appendTo(activeSection.firstElementChild);
				...
				case "bulletList":
					$(`<ul class="unordered-list text-justify col-lg-12" type="disc" style="margin-top: 1.5rem !important;">
							<li class="col-lg-3 text-justify col-lg-7">
								<div class="row text-justify col-lg-12 col-sm-12 list-content-container">
									<div contenteditable class="col-lg-8 col-sm-6 text-justify list-value">
										<!-- <textarea class="list-textarea col-lg-12"></textarea> -->
									</div>
									<div class="options-container col-lg-4 col-sm-5 list-options">
										<span class="option-icon col-lg-2 col-sm-1 list-item-next"><i class="fa fa-plus" aria-hidden="true"></i></span>
										<span class="option-icon col-lg-2 col-sm-1 list-item-image"><i class="fa fa-image" aria-hidden="true"></i></span>
										<span class="option-icon col-lg-2 col-sm-1 list-item-ordered-list"><i class="fa fa-list-ol" aria-hidden="true"></i></span>
										<span class="option-icon col-lg-2 col-sm-1 list-item text-edit"><i class="fa fa-font" aria-hidden="true"></i></span>
							        	<span class="option-icon col-lg-2 col-sm-1 list-item-delete"><i class="fa fa-trash" aria-hidden="true"></i></span>
							        </div>
							    </div>
	...
	// Swaps row values with the values of the row below it
	...
	// Swaps row values (textarea.value) with the values of the row above it
	function tableRowUp() {
		const currentRowValues = [...this.parentElement.parentElement.querySelectorAll(".table-value")];
		...
		// Place the next row's textarea values into the nextRowValuesText
		previousRowValues.forEach(value => {
			previousRowValuesText.push($.trim($(value).text()));
		});

		// Place the current row's values into the previous row's values
		for(var index = 0; index < currentRowValues.length; index++)
			$(previousRowValues[index]).text($.trim($(currentRowValues[index]).text()));

		// Place the previous row's values into the current row's values
	...
	// Deletes the whole row from the table
	function tableRowDelete() {
		...
		$(this).parent().parent().remove();		// Remove selected table row
	...
	// Increments image width by 10 percent (Relative to the image container width)
	function imageGrow() {
	...
	// Decrements image width by 10 percent (Relative to the image container width)
	...
	// Deletes whole image container
	function deleteElement() {
	// const image = this.parentElement.parentElement.previousElementSibling.children[0].children[0];
  	// const index = imageArray.indexOf(document.getElementById(this.id));
  	// console.log(this.parentElement.parentElement.previousElementSibling.children[0].children[0].id);
  	// if(index > -1)
	...
	function addNextListItem(element) {
		...
		// remove all options from current "options-container" except for delete option
		// removeExtraOptions(element);
		updateList();
		updateTextEditPopover();
	}

	function addListImageBlock() {
		$(`<div class="row col-lg-12" style="border: none; margin: 2.5rem auto">
	       		<div class="list-image row text-center col-lg-10 list-image-container">
	       			<div class="list-image col-lg-12">
	                	<input class="upload-image" type="file" />
	            	</div>
	        	</div>
	...
	function continueOrderedListItem() {
		// Add new List element (with new options) after the final list element
		$(`<li class="clear-float col-lg-3 text-justify col-lg-7">
				<div class="row text-justify col-lg-12 list-content-container">
					<div contenteditable class="col-lg-8 col-sm-6 text-justify list-value">
						<!-- <textarea class="list-textarea col-lg-12"></textarea> -->
					</div>
	...
	function deleteListItem() {
		...
		if(this.parentElement.parentElement.parentElement.parentElement.children.length > 1 && optionsContainerChildren.length > 1) {
			...
			// $(previousListElement.firstElementChild.firstElementChild.nextElementSibling).prepend(optionsContainerChildren[2]);
			// $(previousListElement.firstElementChild.firstElementChild.nextElementSibling).prepend(optionsContainerChildren[1]);
			// $(previousListElement.firstElementChild.firstElementChild.nextElementSibling).prepend(optionsContainerChildren[0]);
	...
	function updateSections() {
		// $(".new-section").unbind("focus").focus(function() {
  ...
  //        	$(section).sortable({
	...
	function updateTableOptions() {
		...
		$(".row-down").unbind("click", tableRowDown).click(tableRowDown);
		$(".row-up").unbind("click", tableRowUp).click(tableRowUp);
		$(".row-delete").unbind("click", tableRowDelete).click(tableRowDelete);

		// $(".table-textbox").unbind("focus").focus(function() {
		// 	$(this).unbind("focusout").focusout(updateTableTextbox);
		// });

		// $(".table-value").unbind("dblclick", tableTextboxInteraction).dblclick(tableTextboxInteraction);
	}
	...
	function updateTextbox() {
		// $(".textbox-textarea").unbind("focus").focus(function() {
		// 	$(this).focusout(updateTextboxText);
		// });

		// $(".textarea-container").unbind("dblclick", textboxInteraction).dblclick(textboxInteraction);
	...
	function updateImageOptions() {
		/*$(".upload-image").unbind("change").change(function() {
	        ...
	        $("#imageSubmit").click(function() {
			    ...
			    $(`<img class="image" id="image${imageIDNumber}" src="#" alt="your image" width="${width}%" height="${height}%" />`).insertAfter(thisImage);
			    addImage(thisImage);
			    $(thisImage).remove();
			    $(this).unbind("click");
	        });
	...
	function updateMessages() {
		// $(".important-text, .note-text").unbind("focus").focus(function() {
		// 	$(this).focusout(updateMessage);
		// });

		// $(".important-message-container, .note-message-container").unbind("dblclick", messageInteraction).dblclick(messageInteraction);
	...
	function updateList() {
		...
		// $(".list-value").unbind("dblclick", listInteraction).dblclick(listInteraction);
	...
	function updateTextEditPopover() {
		...
		// 	$("#textEditPopover").css({ "top": (e.pageY - $("#textEditPopover").height() / 1.5), "left": "25%", "position": "absolute" });
		// 	clearTimeout(hidePopupTimeoutId);
		// 	$("#textEditPopover").show();
		// });

		// $("[contenteditable]").on("mouseout", function() {
		...
		// $(".text-edit").unbind("hover").hover(function(e) {
		...
		$(".text-edit").unbind("mousedown").mousedown(function(e) {
			...
			$(".text-color").val("#000000");
			$(".font-size-input").val($(element).css("font-size").substring(0, $(element).css("font-size").length - 2));
			$("#textEditPopover").css({ "top": (e.pageY - $("#textEditPopover").height() / 1.475), "left": "20%", "position": "absolute", "z-index": 1 });
			$("#textEditPopover").disableSelection().draggable().show();
		});
	...
	function removeExtraOptions(optionElement) {
		const optionsContainerChildren = $(optionElement).parent().children();		// Get all the children of the "options-container"
